...
class single_snapshot_partner:
    ...
    def __init__(self, filename, dir='.', target_datasets=[], autoanalysis=True, info=True):
        ...
        for target in self.target_datasets:
            if not (target in self.dataset_keys):
                raise KeyError("dataset <{}> not found in the file <{}>".format(
                    target, dir+'/'+filename))
        ...
        self.get_system_center = lambda: self.__system_center
        # the azimuthal angle of major axis, in [rad]
    ...
    def recenter(self, sphere_size=None, box_size=None, MAXLOOP=1000):
        ...
        while (variation > criterion and loop < MAXLOOP):
            ...
            if sphere_size:
                index = np.where(np.linalg.norm(
                    self.__coordinates - self.__system_center, axis=1, ord=2) <= sphere_size)[0]
                # index: inside a sphere
            elif box_size:
                index = np.where(
                    np.abs((self.__coordinates - self.__system_center)[:, 0]) <= box_size/2)[0]
                index = index[np.where(
                    np.abs((self.__coordinates - self.__system_center)[index, 1]) <= box_size/2)[0]]
                index = index[np.where(
                    np.abs((self.__coordinates - self.__system_center)[index, 2]) <= box_size/2)[0]]
            else:
                # the full index
                index = [i for i in range(len(self.__coordinates))]

            if self.has_potential():
                potentials_sorted = self.__potentials*1.0
                potentials_sorted.sort()
                bounded_boundary = potentials_sorted[int(
                    len(potentials_sorted) * 0.01) - 1]
                # the boundary of the most bounded particles
                index = np.array(index)[np.where(
                    self.__potentials[index] <= bounded_boundary)[0]]

            if self.has_mass():
                cur_coord = self.__coordinates[index, :]
                cur_mass = self.__masses[index]
                new = np.sum(cur_coord * np.column_stack((cur_mass,
                             cur_mass, cur_mass)), axis=0) / np.sum(cur_mass)
            else:
                cur_coord = self.__coordinates[index, :]
    ...
    def calculate_cylindrical_coordinates(self):
        ...
        Rs = np.linalg.norm((self.__coordinates - self.__system_center)
                            [:, :2], axis=1, ord=2)  # R of (R, Phi, z)
        ...
        if 0 in Rs:
            ...
            Phis = np.arctan2(self.__coordinates[index, 1] - self.__system_center[1],
                              self.__coordinates[index, 0] - self.__system_center[0])
        else:
            Phis = np.arctan2(self.__coordinates[:, 1] - self.__system_center[1],
                              self.__coordinates[:, 0] - self.__system_center[0])

        # normalized the range to [0, 2pi]
        Phis[np.where(Phis < 0)[0]] += np.pi*2
    ...
    def calculate_bar_strength(self, region_size=6):
        ...
        if self.has_mass():
            numerator = (self.__masses[index] * np.exp(2j *
                         self.__cylindrical_coordiantes[index, 1])).sum()
    ...
    def calculate_bar_major_axis(self, region_size=3, binnum=180):
        ...
        over_density_index = np.where(statistic > mean+std)[0]
    ...
    def __exclude_one_sigma(self, data):
        ...
        if len(data.shape) != 1:
            raise ValueError(
                "Only allow 1-D array in this function(exclude_one_sigma)")
        mean = np.mean(data)
        std = np.std(data)
...
class snapshots_partner:
    ...
    def __init__(self, filenames, dir=".", target_datasets=[], info=False):
        ...
        self.__bar_strengths = tuple(snapshot.get_bar_strength(
        ) for snapshot in self.__snapshots)  # bar strength parameters
        # secure API to return bar strength parameters
        self.get_bar_strengths = lambda: self.__bar_strengths
        self.__buckling_strengths = tuple(snapshot.get_buckling_strength(
        ) for snapshot in self.__snapshots)  # buckling strengths
        self.get_buckling_strengths = lambda: self.__buckling_strengths
        self.__bar_major_axes = tuple(snapshot.get_bar_major_axis(
        ) for snapshot in self.__snapshots)  # bar major axis
    ...
    def calculate_bar_pattern_speeds(self, start=0, end=None):
        ...
        times = np.arange(self.get_snapshot_count()) / \
            (self.get_snapshot_count() - 1) * (end - start) + start
        ...
        if N_positive >= N_negative:
            ...
            bar_pattern_speeds[index_negative] = (
                delta_phis[index_negative] + np.pi) / age_bin
        else:
            # clockwise ration
            bar_pattern_speeds[index_positive] = (
                delta_phis[index_positive] - np.pi) / age_bin
